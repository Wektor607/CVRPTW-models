...
def main():
    ...
    for num in range(0, 21):
        final_res = []
        name_file = f"50/Example{num}.csv"
        ...
        capacity = 750
        Q_max    = capacity # Для 20:500, 50:750, 100:1000
        ...
        for count_vehicles in range(1, 10):
            ...
            model.Params.TimeLimit = 1000 # seconds
            model.Params.LogFile= "Gurobi_50_dist.txt"
            model.optimize()
            ...
            if(model.status == GRB.OPTIMAL):
                ...
                #         f.write(name_file.split('/')[1].split('.csv')[0] + ' ' + str(s) + ' ' + str(model.Runtime) + '\n')
                #     f.close()
                #     with open(f"20TownsResult/Result_dist.csv", "a") as f:
                #         f.write(name_file.split('/')[1].split('.csv')[0] + ' ' + str(r) + ' ' + str(model.Runtime) + '\n')
                #     f.close()

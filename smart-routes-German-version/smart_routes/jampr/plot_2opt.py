...
def plot_route(data, pi, costs, title, idx_in_batch=0, is_tensor=True):
    ...
    layout = go.Layout(title=dict(text=f'<b>VRP{customer_xy.shape[0]} {title}, Total Length = {cost:.3f}</b>', x=0.5, y=1, yanchor='bottom', yref='paper', pad=dict(b=10)),  # https://community.plotly.com/t/specify-title-position/13439/3
                       ...
                       xaxis=dict(title='X', range=[
                                  0, 1], showgrid=False, ticks='outside', linewidth=1, mirror=True),
                       yaxis=dict(title='Y', range=[
                                  0, 1], showgrid=False, ticks='outside', linewidth=1, mirror=True),
                       showlegend=True,
                       width=750,
                       height=700,
                       ...
                       legend=dict(
                           x=1, xanchor='right', y=0, yanchor='bottom', bordercolor='#444', borderwidth=0)
...
if __name__ == '__main__':
    ...
    pretrained = load_model(args.path, embed_dim=128,
                            n_customer=args.n_customer, n_encode_layers=3)
    ...
    if args.txt is not None:
        ...
        data = []
        for i in range(3):
            elem = [generate_data(device, 1, args.n_customer, args.seed)[
                i].squeeze(0) for j in range(args.batch)]
            data.append(torch.stack(elem, 0))
    print(f'data generate time:{time()-t1}s')
    pretrained = pretrained.to(device)
    pretrained.eval()
    with torch.no_grad():
        costs, _, pi = pretrained(

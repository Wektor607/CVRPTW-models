...
def train(cfg, log_path=None):
    ...
    model = AttentionModel(
        cfg.embed_dim, 256, cfg.n_encode_layers, cfg.n_heads, cfg.tanh_clipping)
    model.train()
    device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
    model.to(device)
    baseline = RolloutBaseline(model, cfg.task, cfg.weight_dir, cfg.n_rollout_samples,
                               cfg.embed_dim, cfg.n_customer, cfg.warmup_beta, cfg.wp_epochs, device, batch_size=cfg.batch)
    ...
    for epoch in range(cfg.epochs):
        ...
        if not os.path.isdir(cfg.weight_dir):
            os.mkdir(cfg.weight_dir)
        torch.save(model.state_dict(), '%s%s_epoch%s.pt' %
                   (cfg.weight_dir, cfg.task, epoch))

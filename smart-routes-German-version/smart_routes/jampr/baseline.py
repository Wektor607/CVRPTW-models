...
def load_model(path, embed_dim=128, n_customer=20, n_encode_layers=3):
    ...
    model_loaded = AttentionModel(
        embed_dim=embed_dim, n_encode_layers=n_encode_layers, n_heads=8, tanh_clipping=10., FF_hidden=512)
...
class RolloutBaseline:
    def __init__(self, model, task, weight_dir, n_rollout_samples=10000,
        ...
        self._update_baseline(model, epoch, batch_size)

    def _update_baseline(self, model, epoch, batch_size=512):

        # Load or copy baseline model based on self.from_checkpoint condition
        if self.from_checkpoint and self.alpha == 0:
            print('Baseline model loaded')
            self.model = self.load_model(
                self.path_to_checkpoint, embed_dim=self.embed_dim, n_customer=self.n_customer)
        else:
            ...
            torch.save(self.model.state_dict(), '%s%s_epoch%s.pt' %
        ...
        self.dataset = Generator(
            self.device, n_samples=self.n_rollout_samples, n_customer=self.n_customer)

        print(
            f'Evaluating baseline model on baseline dataset (epoch = {epoch})')
        self.bl_vals = self.rollout(self.model, self.dataset, batch=batch_size).cpu().numpy()
    ...
    def eval_all(self, dataset, batch):
        ...
        val_costs = self.rollout(self.model, dataset, batch=batch)
    ...
    def epoch_callback(self, model, epoch):
        ...
        candidate_vals = self.rollout(model, self.dataset).cpu().numpy()
        candidate_mean = candidate_vals.mean()

        print(
            f'Epoch {self.cur_epoch} candidate mean {candidate_mean}, baseline mean {self.mean}')

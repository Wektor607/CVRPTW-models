...
class Env():
    def __init__(self, x, node_embeddings, K=10, max_tour_len=20):
        super().__init__()
        """ depot_xy: (batch, 2)
        ...
        self.depot_xy, customer_xy, self.demand = x[:3]
        self.depot_xy, customer_xy, self.demand = self.depot_xy.to(
            self.device), customer_xy.to(self.device), self.demand.to(self.device)
        self.xy = torch.cat(
            [self.depot_xy[:, None, :], customer_xy], 1).to(self.device)
        self.node_embeddings = node_embeddings
        self.batch, self.n_nodes, self.embed_dim = node_embeddings.size()

        self.is_next_depot = torch.ones(
            [self.batch, 1], dtype=torch.bool).to(self.device)
        self.visited_customer = torch.zeros(
            (self.batch, self.n_nodes-1, 1), dtype=torch.bool).to(self.device)

        self.k = K
        self.max_tour_len = max_tour_len

    def init_vehicle(self):
        vehicle = torch.zeros((self.batch, self.k, 5))
    ...
    def get_mask_D(self, next_node, visited_mask, D):
        ...
        customer_idx = torch.argmax(
            visited_mask[:, 1:, :].type(torch.long), dim=1)
        selected_demand = torch.gather(
            input=self.demand, dim=1, index=customer_idx)
        D = D - selected_demand * (1.0 - self.is_next_depot.float())
        capacity_over_customer = self.demand > D
        mask_customer = capacity_over_customer[:,
                                               :, None] | self.visited_customer
        mask_depot = self.is_next_depot & (
            torch.sum((mask_customer == False).type(torch.long), dim=1) > 0)
    ...
    def _get_step(self, next_node, D):
        ...
        visited_mask = one_hot.type(
            torch.bool).permute(0, 2, 1).to(self.device)

        mask, D = self.get_mask_D(next_node, visited_mask, D)
        self.demand = self.demand.masked_fill(
            self.visited_customer[:, :, 0] == True, 0.0)

        prev_node_embedding = torch.gather(
            input=self.node_embeddings, dim=1, index=next_node[:, :, None].repeat(1, 1, self.embed_dim))
    ...
    def create_mask_t1(self):
        ...
        mask_depot = torch.ones(
            [self.batch, 1, 1], dtype=torch.bool).to(self.device)
    ...
    def create_context_D_t1(self):
        ...
        depot_idx = torch.zeros([self.batch, 1], dtype=torch.long).to(
            self.device)  # long == int64
        depot_embedding = torch.gather(
            input=self.node_embeddings, dim=1, index=depot_idx[:, :, None].repeat(1, 1, self.embed_dim))
    ...
    def get_costs(self, pi):
        ...
        """
        d = torch.gather(input=self.xy, dim=1,
                         index=pi[:, :, None].repeat(1, 1, 2))
...
class TopKSampler(Sampler):
    def forward(self, logits):
        # == torch.argmax(log_p, dim = 1).unsqueeze(-1)
...
class CategoricalSampler(Sampler):
    def forward(self, logits):
        ...
        for i in range(logits.shape[0]):
            r.append(torch.multinomial(logits[i].exp(), self.n_samples)[None, ...])
        return torch.cat(r)

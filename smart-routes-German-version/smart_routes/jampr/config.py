...
def arg_parser():
    ...
    parser.add_argument('-m', '--mode', metavar='M', type=str,
                        default='train', choices=['train', 'test'], help='train or test')
    ...
    parser.add_argument('-n', '--n_customer', metavar='N', type=int,
                        default=20, help='number of customer nodes, time sequence')

    # train config
    parser.add_argument('-b', '--batch', metavar='B',
                        type=int, default=512, help='batch size')
    parser.add_argument('-bs', '--batch_steps', metavar='BS', type=int,
                        default=2500, help='number of samples = batch * batch_steps')
    parser.add_argument('-bv', '--batch_verbose', metavar='BV', type=int,
                        default=10, help='print and logging during training process')
    parser.add_argument('-nr', '--n_rollout_samples', metavar='R',
                        type=int, default=10000, help='baseline rollout number of samples')
    parser.add_argument('-e', '--epochs', metavar='E', type=int, default=20,
                        help='total number of samples = epochs * number of samples')
    parser.add_argument('-em', '--embed_dim', metavar='EM',
                        type=int, default=128, help='embedding size')
    parser.add_argument('-nh', '--n_heads', metavar='NH',
                        type=int, default=8, help='number of heads in MHA')
    parser.add_argument('-c', '--tanh_clipping', metavar='C', type=float,
                        default=10., help='improve exploration; clipping logits')
    parser.add_argument('-ne', '--n_encode_layers', metavar='NE',
                        type=int, default=3, help='number of MHA encoder layers')
    ...
    parser.add_argument('-wb', '--warmup_beta', metavar='WB', type=float,
                        default=0.8, help='exponential moving average, warmup')
    parser.add_argument('-we', '--wp_epochs', metavar='WE',
                        type=int, default=1, help='warmup epochs')

    parser.add_argument('--islogger', action='store_false',
                        help='flag csv logger default true')
    parser.add_argument('-ld', '--log_dir', metavar='LD',
                        type=str, default='./Csv/', help='csv logger dir')
    parser.add_argument('-wd', '--weight_dir', metavar='MD',
                        type=str, default='./Weights/', help='model weight save dir')
    parser.add_argument('-pd', '--pkl_dir', metavar='PD',
                        type=str, default='./Pkl/', help='pkl save dir')
    parser.add_argument('-cd', '--cuda_dv', metavar='CD',
                        type=str, default='0', help='os CUDA_VISIBLE_DEVICE')
...
def dump_pkl(args, verbose=True, param_log=True):
    ...
    with open(cfg.pkl_path, 'wb') as f:
        ...
        if param_log:
            path = '%sparam_%s_%s.csv' % (
                cfg.log_dir, cfg.task, cfg.dump_date)  # cfg.log_dir = ./Csv/
...
def train_parser():
    ...
    parser.add_argument('-p', '--path', metavar='P', type=str,
                        default='PyTorchJAMPR/Pkl/VRP20_train.pkl',

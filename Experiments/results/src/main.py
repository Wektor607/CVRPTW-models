...
def main():
    ...
    while(i < len(lst)):
        ...
        with open(f'current_result/LKH_CVRPTW_result.txt', 'r') as res_file:
            data = res_file.read()
            with open(f'results/LKH_RES_CVRPTW{count_towns-1}_penalty.txt', 'a') as write_file:
                write_file.write(data)
        
        if(f'results/LKH_RES_CVRPTW{count_towns-1}_penalty.txt' in lst_results):
            lst_results.remove(f'results/LKH_RES_CVRPTW{count_towns-1}_penalty.txt')
            lst_results.append(f'results/LKH_RES_CVRPTW{count_towns-1}_penalty.txt')
        else:
            lst_results.append(f'results/LKH_RES_CVRPTW{count_towns-1}_penalty.txt')

...
int read_file_tw(const char* name_file, twtown *towns, int counttowns)
{
    ...
    for(int i = counttowns-1; i >= 0 ; i--)
    {
        ...
        towns[i] = maketwtown(maketown(i, x, y, cap), (hst * 60 + mst) * 60, (het * 60 + met) * 60, st);
    }
    fclose(in);
    return 0;
}
...
//TODO: Сделать файл с матрицей расстояний и считывать из нее расстояния и класть тут в dist
...
void parseOneTwTownNoIndex(const char pathFile[], const char matrixFile[],const char newFileName[], int tcountTowns)
{
    ...
    int64_t ** time_arr = (int64_t **)malloc(tcountTowns * sizeof(int64_t *));
    if (time_arr == NULL){
        printf("Allocation error\n");
    }
    for(int i = 0; i < tcountTowns; ++i){
        time_arr[i] = (int64_t *)malloc(tcountTowns * sizeof(int64_t));
        if (time_arr[i] == NULL){
            printf("Allocation error\n");
        }
    }
    for(int i = 0; i < tcountTowns+1; ++i){
        if (i != 0){
            for(int j = 0; j < tcountTowns; ++j){
                fscanf(in, "%ld", &time_arr[i-1][j]);
            }
        }
        else{
            int64_t tmp;

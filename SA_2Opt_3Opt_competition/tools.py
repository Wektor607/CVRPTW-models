...
def validate_dynamic_epoch_solution(epoch_instance, epoch_solution):
    ...
    assert not must_dispatch.any(), "Some requests must be dispatched but were not: {}".format(request_idx[must_dispatch])
...
def validate_route_capacity(route, demands, capacity):
    assert sum(demands[route]) <= capacity, "Capacity validated for route, {} > {}".format(sum(demands[route]), capacity)
...
def validate_route_time_windows(route, dist, timew, service_t, release_t=None):
    ...
    for stop in route:
        ...
        assert current_time <= latest_arrival, "Time window violated for stop {}: {} not in ({}, {})".format(stop, current_time, earliest_arrival, latest_arrival)
        current_time += service_t[stop]
        prev_stop = stop
    current_time += dist[prev_stop, depot]
    assert current_time <= latest_arrival_depot, "Time window violated for depot: {} not in ({}, {})".format(current_time, earliest_start_depot, latest_arrival_depot)
